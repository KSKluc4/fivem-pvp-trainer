...
from services.aim_level import recommended_difficulty, CATEGORIES, CATEGORY_DRILLS
...
# drill catalog (src/frontend/src/trainer/catalog.js, mirrored as
# CATEGORY_DRILLS in services.aim_level). Since 3.1.0 the questionnaire's
# aim_difficulty/reflex_level answers pick CATEGORIES; the concrete drill
# within each chosen category rotates deterministically by date (see
# _drill_for_category) so consecutive days don't repeat the same exercise.
# Each drill's difficulty comes from the user's own per-CATEGORY aim level
# (services.aim_level), the same system the in-app trainer displays.

# Emphasis bonus per questionnaire answer — higher score sorts first and is
# more likely to make the cut when only 2-3 categories fit the time budget.
AIM_DIFFICULTY_EMPHASIS = {
    'tracking': {'tracking': 2},
    'flick':    {'flicking': 2, 'clicking': 1},
    'close':    {'precision': 2, 'clicking': 1},
}
REFLEX_EMPHASIS = {
    'lento': {'reaction': 2},
}
...
def _category_priority(aim_difficulties, reflex_level: str, today: date) -> list:
    """The 5 drill categories ordered by how much today's profile emphasizes
    them (highest first); ties keep the catalog's own category order.
    `aim_difficulties` may hold 1 or 2 values (SPEC-004)."""
    ...
    for aim_difficulty in aim_difficulties:
        ...
        for cat, bonus in AIM_DIFFICULTY_EMPHASIS.get(aim_difficulty, {}).items():
            scores[cat] = scores.get(cat, 1) + bonus * weight
    for cat, bonus in REFLEX_EMPHASIS.get(reflex_level, {}).items():
        scores[cat] = scores.get(cat, 1) + bonus
    return sorted(CATEGORIES, key=lambda cat: (-scores[cat], CATEGORIES.index(cat)))


def _drill_for_category(category: str, today: date) -> str:
    """Which of the category's drills today's routine uses. Deterministic for
    a given (day, category) — a refresh never reshuffles an already-generated
    routine — but rotates across days so the same category doesn't hand out
    the same drill every single day."""
    drills = CATEGORY_DRILLS[category]
    rng = random.Random(f'{today.isoformat()}|{category}')
    return rng.choice(drills)
...
def _build_aim_block(profile: dict, aim_levels: dict, today: date = None):
    ...
    trainer drill id (a trainer/catalog.js entry), used directly as the
    "Treinar" deep-link target. `name` is a routine-wide unique key (warmup
    and main never collide even when they pick the same drill) used for
    progress tracking on the frontend.

    v3.1.0: selection happens per CATEGORY — the profile's foci pick which
    categories train today, _drill_for_category picks the day's concrete
    drill within each, and `aim_levels` is keyed by category."""
    ...
    priority    = _category_priority(aim_difficulties, profile.get('reflex_level', ''), today)
    drill_count = _main_drill_count(daily_time)

    # SPEC-004: reserve 1 slot per distinct aim_difficulty (highest-scoring
    # category within that focus's own emphasis set) before filling the rest
    # by overall score — a structural guarantee that both chosen foci show up
    # in treino_principal even if reflex_level's bonus would otherwise crowd
    # one out. With 0-1 values this reserves at most 1 slot, which is always
    # already priority[0] — same shape as `priority[:drill_count]`.
    reserved = []
    for aim_difficulty in aim_difficulties:
        target_categories = set(AIM_DIFFICULTY_EMPHASIS.get(aim_difficulty, {}))
        best = next((cat for cat in priority if cat in target_categories and cat not in reserved), None)
        if best:
            reserved.append(best)
    main_categories = (reserved + [cat for cat in priority if cat not in reserved])[:drill_count]
    rounds = _rounds_for_budget(daily_time, drill_count)

    primary        = _primary_aim_difficulty(aim_difficulties, today)
    top_category   = next((cat for cat in priority if cat in AIM_DIFFICULTY_EMPHASIS.get(primary, {})), priority[0])
    top_drill      = _drill_for_category(top_category, today)
    top_difficulty = recommended_difficulty(aim_levels.get(top_category))
    warmup = [{
        'name':       f'aquecimento_{top_drill}',
        'exercise':   top_drill,
    ...
    for category in main_categories:
        drill = _drill_for_category(category, today)
        main.append({
            'name':       drill,
            'exercise':   drill,
            'category':   category,
            'difficulty': recommended_difficulty(aim_levels.get(category)),
            'rounds':     rounds,
