...
def test_level_for_single_score_baseline_below_first_threshold():
    assert level_for_single_score('grid_2d', 'medio', 0) == 1
    assert level_for_single_score('grid_2d', 'medio', 19) == 1


def test_level_for_single_score_hits_each_threshold_exactly():
    t = THRESHOLDS_BY_EXERCISE['grid_2d']['medio']
    for level in (2, 3, 4, 5):
        assert level_for_single_score('grid_2d', 'medio', t[level]) == level


def test_level_for_single_score_caps_at_five_above_top_threshold():
    t = THRESHOLDS_BY_EXERCISE['grid_2d']['medio']
    assert level_for_single_score('grid_2d', 'medio', t[5] + 1000) == 5
...
def test_level_for_single_score_unknown_exercise_or_difficulty_is_baseline():
    ...
    assert level_for_single_score('grid_2d', 'not_a_real_difficulty', 999999) == MIN_LEVEL


def test_all_four_exercises_have_thresholds_for_all_three_difficulties():
    assert set(EXERCISES) == {'tracking_2d', 'grid_2d', 'flick_2d', 'micro_2d'}
...
def test_exercise_aim_level_computed_at_min_attempts():
    scores = score_rows('grid_2d', 'medio', [50] * MIN_ATTEMPTS)  # well above level-5 threshold (40)
...
def test_exercise_aim_level_only_considers_recent_window():
    ...
    recent = score_rows('grid_2d', 'medio', [5] * RECENT_WINDOW)
    older  = score_rows('grid_2d', 'medio', [999] * 5)
...
def test_exercise_aim_level_averages_mixed_difficulties():
    ...
    scores = (
        score_rows('grid_2d', 'medio', [50] * 3)
        + score_rows('grid_2d', 'facil', [10] * 2)
    )
...
def test_overall_aim_level_none_when_nothing_computed():
    assert overall_aim_level({'grid_2d': None, 'flick_2d': None}) is None


def test_overall_aim_level_averages_only_non_none_exercises():
    assert overall_aim_level({'grid_2d': 4, 'flick_2d': None, 'micro_2d': 2}) == 3.0
...
def test_accelerator_false_without_enough_data(mock_get, mock_upsert):
    result = resolve_aim_accelerator(1, per_exercise_levels={'grid_2d': None})
...
def test_accelerator_false_on_first_ever_resolution(mock_get, mock_upsert):
    ...
    result = resolve_aim_accelerator(1, per_exercise_levels={'grid_2d': 3})
    assert result is False
