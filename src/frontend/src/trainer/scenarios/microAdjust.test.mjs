...
import { EXERCISE_ID, SESSION_DURATION_S, DIFFICULTIES, createTarget } from './microAdjust.js'
...
test('exercise metadata', () => {
  assert.equal(EXERCISE_ID, 'micro_adjust')
  assert.equal(SESSION_DURATION_S, 60)
  assert.deepEqual(Object.keys(DIFFICULTIES), ['facil', 'medio', 'dificil'])
})

test('both radius and timeout shrink as difficulty increases', () => {
...
test('spawn distance stays close to the crosshair (tighter than Quick Flick)', () => {
  for (const key of Object.keys(DIFFICULTIES)) {
    assert.ok(DIFFICULTIES[key].distanceRange[1] <= 0.7)
  }
})
...
test('createTarget spawns inside ARENA_BOUNDS regardless of camera orientation', () => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const target = createTarget(scene, 'facil', camera)
